...
public class InviteHandleQueries {
    ...
    private int[] getRandomNums(int maxVal) {
        int[] randoms = new int[3];
        int counter = 0;
        boolean exists = false;

        while (counter < 3) {
            int random = (int) (Math.random() * (maxVal - 1) + 1);
    ...
    public void setupGame(String color) {
        ...
        int[] randomToolcards = getRandomNums(13);
        for (int i = 0; i < 3; i++) {
        ...
        int[] randomObjectiveCards = getRandomNums(11);
        for (int i = 0; i < 3; i++) {
